  ...
  <view class="page pb-120">
    ...
    <view class="section card mt-24">
      ...
      <view class="prog-row" v-for="(i,idx) in weekNutri" :key="idx">
        ...
        <view class="bar"><view class="bar-inner" :style="{ width: i.value+'%', background: i.color }"></view></view>
        <text class="prog-val">{{i.value}}%</text>
      </view>
      <text class="tip">💡 点击日历中日期查看当日食物记录</text>
    </view>
...
export default {
  ...
  data() {
    return {
      ...
      days: this.buildDays(),
      weekNutri: [
        { name: '蛋白质', value: 78, color: '#21C87A' },
        { name: '维生素', value: 65, color: '#5C8DFF' },
        { name: '纤维', value: 92, color: '#FF8C2B' }
      ],
      sheetVisible: false,
      sheetTitle: ''
    }
...
.arrow { font-size: 40rpx; color:#888 }
...
.tip { display:block; margin-top: 18rpx; color:#818992; font-size: 24rpx; }
</style>